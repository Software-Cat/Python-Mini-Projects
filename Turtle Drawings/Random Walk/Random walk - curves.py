...
def one_dimentional_noise(amount):
    noise = PerlinNoiseFactory(1)
    values = []
    for i in range(amount):
        values.append(noise.get_plain_noise(float(i)/amount))
    return values

class Painter(t.RawTurtle):
    colors = ["red","orange","yellow","green","cyan","blue","purple"]
    _screen = None

    def __init__(self, upto=1000, stepLength=25, restart=False):
        if Painter._screen is None:
            Painter._screen = t.Screen()
        t.RawTurtle.__init__(self, Painter._screen)
        
        self.stepLength = stepLength
        self.upto = upto
        self.directions = one_dimentional_noise(self.upto)
        self.index = 0
        self.speed(0)
        Painter._screen.bgcolor([0, 0, 0])

    def move(self):
        self.color(random.choice(Painter.colors))
        self.setheading(self.directions[self.index]*360000)

        if self.pos()[0] < -t.window_width() / 2: # if inside GUI
            self.setheading(0)
        if self.pos()[0] > t.window_width() / 2:
            self.setheading(180)
        if self.pos()[1] < -t.window_height() / 2:
            self.setheading(90)
        if self.pos()[1] > t.window_height() / 2:
            self.setheading(270)

        self.forward(self.stepLength)
        if self.index == upto-1 and self.restart:
            self.directions = one_dimentional_noise(self.upto)
            self.index = 0
        elif self.index == upto-1 and not self.restart:
            pass
        else:
            self.index += 1
...
upto = 1000
for i in range(3):
    painters.append(Painter(upto=1000))
